...
// Categories mapped from folder names
const CATEGORY_META: Record<string, { label: string; icon: string }> = {
  ...
  'game-development': { label: 'Game Dev',            icon: '🎮' },
  integrations:       { label: 'Integrations',        icon: '🔌' },
  'paid-media':       { label: 'Paid Media',          icon: '📣' },
  'project-management': { label: 'Project Mgmt',     icon: '📋' },
  specialized:        { label: 'Specialized',         icon: '🔬' },
  'spatial-computing':{ label: 'Spatial Computing',   icon: '🥽' },
};
...
export function loadAllAgents(): AgentDefinition[] {
  ...
  // Try to load from agency-agents-main resource folder
  const AGENCY_AGENTS_ROOT = path.join(
    process.cwd(),
    ...
    for (const file of files) {
      try {
        ...
        agents.push({
          ...
          emoji: meta.emoji || CATEGORY_META[category]?.icon || '🤖',
          vibe: meta.vibe || '',
          category,
