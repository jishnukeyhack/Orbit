...
export default function DynamicDashboardPage() {
  ...
  // Authenticate user and verify their encryption key matches the URL
  useEffect(() => {
    let mounted = true;
    supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      setUserId(user.id);
      const userKey = user.user_metadata?.encryption_key;
      
      // If user has a key but it doesn't match this page URL, redirect to correct one
      if (userKey && userKey !== urlKey) {
        router.replace(`/dashboard/${userKey}`);
      } else {
  ...
  // Fetch real stats from SQLite filtered by active user ID
  const fetchData = async (uid: string) => {
    try {
      const [statsRes, runsRes, agentsRes] = await Promise.all([
        fetch(`/api/pipeline?action=stats&userId=${uid}`).then(r => r.json()).catch(() => null),
        fetch(`/api/pipeline?limit=10&userId=${uid}`).then(r => r.json()).catch(() => ({ runs: [] })),
  ...
  useEffect(() => {
    ...
    // Dynamic reactive updates using BroadcastChannel for 100% real-time accuracy
    const updateChannel = new BroadcastChannel('orbit_pipeline_sync');
    updateChannel.onmessage = () => {
      fetchData(userId);
    };

    // Standard high-fidelity backup polling
    const interval = setInterval(() => fetchData(userId), 2500);
  ...
  if (checkingKey) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60vh', gap: '1rem' }}>
        ...
        <span style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>Decrypting secure channel...</span>
        ...
        <div>
          ...
          {runs.length === 0 ? (
            <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '0.875rem', padding: '3rem', textAlign: 'center' }}>
              ...
              <div style={{ color: '#94a3b8', fontSize: '0.95rem' }}>No pipeline runs executed under your account yet.</div>
        ...
        <div>
          ...
          <div style={{ marginTop: '1rem', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
            ...
            {[
              ...
              { label: 'Configure API Keys', href: '/settings' },
            ].map(({ label, href }) => (
