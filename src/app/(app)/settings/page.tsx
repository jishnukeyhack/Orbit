...
export default function SettingsPage() {
  ...
  const saveKey = async () => {
    ...
    // Persist to SQLite settings table (server-side)
    try {
  ...
  const configItems = [
    ...
    { key: 'Failover Model', value: 'gemini-1.5-pro', desc: 'High-availability backup if OpenAI key fails or is missing', icon: '✨' },
  ...
  return (
    <div style={{ padding: '1.5rem', maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: '1.5rem', marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.75rem' }}>
          {[
            ...
            { label: 'Gemini Connection', ok: status?.gemini ?? false, icon: '✨' },
      ...
      {/* Gemini API Key */}
      <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: '1.5rem', marginBottom: '1.5rem' }}>
        <div style={{ fontWeight: 600, color: '#f1f5f9', marginBottom: '0.4rem' }}>✨ Google Gemini API Key</div>
        <div style={{ fontSize: '0.82rem', color: '#64748b', marginBottom: '1.25rem', lineHeight: 1.6 }}>
          Optional secondary LLM. Used automatically when OpenAI is unavailable or fails.
          Get yours at{' '}
          <a href="https://aistudio.google.com/" target="_blank" rel="noreferrer" style={{ color: '#6366f1', textDecoration: 'none' }}>
            aistudio.google.com →
          </a>
        </div>
        ...
        {savedGeminiKey && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 10, marginBottom: '1rem' }}>
            <span style={{ color: '#10b981' }}>✅</span>
            <span style={{ fontSize: '0.85rem', color: '#94a3b8', fontFamily: 'monospace', flex: 1 }}>{savedGeminiKey}</span>
            <button onClick={clearGeminiKey} style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 6, color: '#ef4444', fontSize: '0.72rem', padding: '4px 10px', cursor: 'pointer' }}>
              Remove
            </button>
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: '0.75rem' }}>
          <input
            ...
            placeholder={savedGeminiKey ? 'Enter new key to replace...' : 'AIzaSy...'}
            style={{
              flex: 1, padding: '10px 14px', background: 'rgba(255,255,255,0.05)',
              border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10,
              color: '#f1f5f9', fontSize: '0.88rem', outline: 'none', fontFamily: 'monospace',
        ...
        {testGeminiResult && (
          <div style={{ padding: '10px 14px', borderRadius: 8, background: testGeminiResult.ok ? 'rgba(16,185,129,0.08)' : 'rgba(239,68,68,0.08)', border: `1px solid ${testGeminiResult.ok ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.2)'}`, fontSize: '0.82rem', color: testGeminiResult.ok ? '#10b981' : '#ef4444' }}>
