...
export async function POST(request: NextRequest) {
  ...
  const body = await request.json() as { workflowId?: string; stepId?: string; stepPrompt?: string; stepName?: string; agentRole?: string };
  const { workflowId, stepId, stepPrompt, stepName, agentRole } = body;

  // Single-step execution mode (from UI) - fallback-safe
  if (!workflowId && (stepPrompt || stepName)) {
    const runId = uuidv4();
    const ws = ensureWorkspace();
    const stepDir = path.join(ws, 'workflow-steps', runId.slice(0, 8));
    mkdirSync(stepDir, { recursive: true });

    const promptToUse = stepPrompt || `Perform the "${stepName || 'Task'}" step as a specialized "${agentRole || 'Agent'}".`;
