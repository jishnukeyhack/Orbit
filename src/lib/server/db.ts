...
export function getOrbitStats() {
  ...
  // Calculate total tokens from results saved in DB
  const runs = db.prepare('SELECT result_json FROM pipeline_runs WHERE result_json IS NOT NULL').all() as Array<{ result_json: string }>;
  let tokensUsed = 0;
  for (const r of runs) {
    try {
...
export function getOrbitAnalytics(daysLimit = 14) {
  const db = getDb();
  const daily = [];
  const now = Date.now();
  ...
  for (let i = daysLimit - 1; i >= 0; i--) {
    ...
    `).get(startOfDay.getTime(), endOfDay.getTime()) as { total: number, completed: number, cost: number | null };
  ...
  // 2. Usage breakdown
  const pipelineCount = db.prepare("SELECT COUNT(*) as c FROM pipeline_runs WHERE task_title NOT LIKE '[Swarm]%'").get() as { c: number };
  const swarmCount = db.prepare("SELECT COUNT(*) as c FROM pipeline_runs WHERE task_title LIKE '[Swarm]%'").get() as { c: number };
  const fileCount = db.prepare("SELECT COUNT(*) as c FROM workspace_files").get() as { c: number };
