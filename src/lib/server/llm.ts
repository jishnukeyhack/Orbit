...
function isRealKey(key: string | undefined): boolean {
  ...
  // Reject obvious placeholders and short strings
  if (
    lower.includes('your-') ||
    ...
    try {
      const geminiContents = messages.map(m => {
        ...
        if (m.role === 'tool') {
          ...
          // Strip timestamp suffix like _171638210382 dynamically for all tools
